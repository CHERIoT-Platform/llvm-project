...
struct QualTypeBoolPIKInfo {
  using KeyTy = std::tuple<QualType, bool, PointerInterpretationKind>;

  template <typename T> static KeyTy getKey(const T &N) { return N.getKey(); }

  static unsigned getHashValue(const KeyTy &Key) {
    return llvm::hash_combine(std::get<0>(Key).getAsOpaquePtr(),
...
class ASTContext : public RefCountedBase<ASTContext> {
  ...
  mutable llvm::UniquingSet<LValueReferenceType, QualTypeBoolPIKInfo>
      LValueReferenceTypes;
  mutable llvm::UniquingSet<RValueReferenceType, QualTypeBoolPIKInfo>
  ...
  mutable llvm::ContextualFoldingSet<DependentPointerType, ASTContext &>
      DependentPointerTypes;
...
#include "clang/Basic/BuiltinTemplates.inc"
  ...
  const RecordDecl *getCHERIClassDecl() const;
  QualType getCHERIClassType() const {
    return getCanonicalTagType(getCHERIClassDecl());
  }
