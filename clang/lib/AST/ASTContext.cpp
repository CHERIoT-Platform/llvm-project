...
ASTContext::ASTContext(LangOptions &LOpts, SourceManager &SM,
    ...
    : ConstantArrayTypes(this_(), ConstantArrayTypesLog2InitSize),
      ...
      DependentAddressSpaceTypes(this_()), 
      DependentPointerTypes(this_()),
      DependentVectorTypes(this_()),
...
QualType ASTContext::getPointerType(QualType T, PointerInterpretationKind PIK) const {
  ...
  // structure.
  llvm::FoldingSetInsertToken Token;
  if (PointerType *PT = PointerTypes.lookup({T, PIK}, Token))
  ...
  if (!T.isCanonical()) {
    ...
    assert(!PointerTypes.lookup({T, PIK}, Token) && "Shouldn't be in the map!");
  }
...
ASTContext::getLValueReferenceType(QualType T, bool SpelledAsLValue, PointerInterpretationKind PIK) const {
  ...
  if (LValueReferenceType *RT =
          LValueReferenceTypes.lookup({T, SpelledAsLValue, PIK}, Token))
  ...
  if (!SpelledAsLValue || InnerRef || !T.isCanonical()) {
    ...
    assert(!LValueReferenceTypes.lookup({T, SpelledAsLValue, PIK}, Token) &&
...
QualType ASTContext::getRValueReferenceType(QualType T, PointerInterpretationKind PIK) const {
  ...
  // structure.
  llvm::FoldingSetInsertToken Token;
  if (RValueReferenceType *RT = RValueReferenceTypes.lookup({T, false, PIK}, Token))
    return QualType(RT, 0);
  ...
  if (InnerRef || !T.isCanonical()) {
    ...
    assert(!RValueReferenceTypes.lookup({T, false, PIK}, Token) &&
           "Shouldn't be in the map!");
  }
