...
public:
  ...
  unsigned getAlignment() {
    assert(isScalar() && "Not a scalar!");
    return reinterpret_cast<uintptr_t>(Vals.second);
  }

  /// getComplexVal - Return the real/imag components of this complex value.
  ///
  std::pair<llvm::Value *, llvm::Value *> getComplexVal() const {
    assert(isComplex() && "Not a complex value!");
  ...
  static RValue get(llvm::Value *V, unsigned Alignment = 0) {
    ...
    ER.Vals.first = V;
    ER.Flavor = Scalar;
    ER.Vals.second = reinterpret_cast<llvm::Value *>(Alignment);
    ER.IsVolatile = false;
    return ER;
  }
  static RValue get(Address Addr, CodeGenFunction &CGF, unsigned Alignment = 0) {
    return RValue::get(Addr.emitRawPointer(CGF), Alignment);
  }
