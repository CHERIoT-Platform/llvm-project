...
llvm::MDNode *CodeGenTBAA::getAnyPtr(unsigned PtrDepth) {
  ...
  if (AnyPtrs.size() < PtrDepth) {
    ...
    // XXX CHERI: Is this the correct address space in general?
    unsigned AS = Module.getDataLayout().getDefaultGlobalsAddressSpace();
    auto Size = Module.getDataLayout().getPointerSize(AS);
