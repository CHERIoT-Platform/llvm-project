  ...
  if (CHERIMethodSuffixAttr *Attr = NewFD->getAttr<CHERIMethodSuffixAttr>()) {
    ...
    for (NamedDecl *D : Defs)
      if (RecordDecl *RD = dyn_cast<RecordDecl>(D))
        CHERIClassTy = Context.getCanonicalTagType(RD);
        ...
        if (NumFields > 0)
          if ((Padding > 8) || ((Padding * 3) > (UnpaddedSize * 4)))
            getDiagnostics().Report(RD->getLocation(),
                ...
                << Context.getCanonicalTagType(RD) << Padding << Size;
      }
  }
...
void Sema::ActOnFields(Scope *S, SourceLocation RecLoc, Decl *EnclosingDecl,
  ...
  if (Record && Record->hasAttr<PackedAttr>() && !Record->isDependentType()) {
      ...
      [&](const RecordDecl *R) {
        for (const auto *F : R->fields()) {
          ...
          if (FTy->isCHERICapabilityType(getASTContext()))
            return true;
          if (FTy->isRecordType() &&
              contains_capabilities(FTy->getAs<RecordType>()->getOriginalDecl()))
            return true;
        }
        return false;
      };
    ...
    if (CheckForUseInArray) {
      ...
      unsigned RecordAlign = Context.getTypeAlign(Context.getCanonicalTagType(Record));
      unsigned RecordSize = Context.getTypeSize(Context.getCanonicalTagType(Record));
...
void Sema::ActOnPragmaOpaque(IdentifierInfo *TypeName, IdentifierInfo *KeyName,
