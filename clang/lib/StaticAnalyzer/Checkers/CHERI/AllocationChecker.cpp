...
bool isAllocation(const MemRegion *R, const AllocationChecker *Chk, ProgramStateRef State) {
  if (!Chk->ReportForUnknownAllocations) {
    const MemSpaceRegion *MemSpace = R->getMemorySpace(State);
...
ExplodedNode *AllocationChecker::emitAllocationPartitionWarning(
  ...
  if (ExplodedNode *ErrNode = C.generateNonFatalErrorNode()) {
    ...
    const MemSpaceRegion *MS = PrevAlloc
                                   ? PrevAlloc->getMemorySpace(C.getState())
