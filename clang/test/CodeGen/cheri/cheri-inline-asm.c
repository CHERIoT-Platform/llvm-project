...
// CHECK-NEXT:    call addrspace(0) void asm sideeffect "# ASM EXPR: $0", "rm,~{memory},~{$1}"(ptr addrspace(200) [[TMP0]]) #[[ATTR1:[0-9]+]]
// CHECK-NEXT:    [[TMP1:%.*]] = load ptr addrspace(200), ptr addrspace(200) [[P_ADDR]], align 16
// CHECK-NEXT:    call addrspace(0) void asm sideeffect "# ASM EXPR: $0", "r,~{memory},~{$1}"(ptr addrspace(200) [[TMP1]]) #[[ATTR1]]
// CHECK-NEXT:    call addrspace(0) void asm sideeffect "# ASM EXPR: $0", "*m,~{memory},~{$1}"(ptr addrspace(200) elementtype(ptr addrspace(200)) [[P_ADDR]]) #[[ATTR1]]
// CHECK-NEXT:    ret void
//
void f(char *p) {
