...
// CHECK: define dso_local chericcallee [2 x i32] @_Z8InitIntsv() local_unnamed_addr addrspace(200) #1 {
__attribute__((cheri_compartment("example"), noinline)) struct TwoIntegers InitInts(void) {
  // CHECK: entry:
  // CHECK:   %call = tail call chericcallee i32 @_Z8GetValuev()
  // CHECK:   %call1 = tail call chericcallee i32 @_Z8GetValuev()
...
// CHECK: define dso_local chericcallee [2 x i32] @_Z7ChgInts11TwoIntegers([2 x i32] %x.coerce) local_unnamed_addr addrspace(200) #1 {
...
// CHECK: define dso_local chericcallee void @_Z9CheckIntsv() local_unnamed_addr addrspace(200) #2 {
__attribute__((cheri_compartment("example"))) void CheckInts() {

  static struct TwoIntegers __attribute__((used)) x = {}; 

  // CHECK: entry:
  // CHECK:   %call = tail call chericcallee [2 x i32] @_Z8InitIntsv()
  // CHECK:   %call1 = tail call chericcallee [2 x i32] @_Z7ChgInts11TwoIntegers([2 x i32] %call)
...
// CHECK: define dso_local chericcallee %struct.TwoPointers @_Z8InitPtrsv() local_unnamed_addr addrspace(200) #1
...
// CHECK: define dso_local chericcallee %struct.TwoPointers @_Z7ChgPtrs11TwoPointers(ptr addrspace(200) %x.coerce0, ptr addrspace(200) %x.coerce1) local_unnamed_addr addrspace(200) #1 {
__attribute__((cheri_compartment("example"), noinline)) struct TwoPointers ChgPtrs(struct TwoPointers x) {
  ...
  // CHECK:   %call2 = tail call chericcallee i32 @_Z8GetValuev()
...
// CHECK: define dso_local chericcallee void @_Z9CheckPtrsv() local_unnamed_addr addrspace(200) #2 {
__attribute__((cheri_compartment("example"))) void CheckPtrs() {

  static struct TwoPointers __attribute__((used)) x = {}; 

  // CHECK: entry:
  // CHECK:   %call = tail call chericcallee %struct.TwoPointers @_Z8InitPtrsv()
  // CHECK:   %0 = extractvalue %struct.TwoPointers %call, 0
  // CHECK:   %1 = extractvalue %struct.TwoPointers %call, 1
  // CHECK:   %call1 = tail call chericcallee %struct.TwoPointers @_Z7ChgPtrs11TwoPointers(ptr addrspace(200) %0, ptr addrspace(200) %1)
...
// Here we want to check that the result is returned by value.
// CHECK:  define dso_local chericcallee %struct.PointerAndInt @_Z10InitPtrIntv() local_unnamed_addr addrspace(200) #1 {
...
// Here we want to check that the struct received as parameter is laid out as two different arguments and, again, is returned by value.
// CHECK: define dso_local chericcallee %struct.PointerAndInt @_Z9ChgPtrInt13PointerAndInt(ptr addrspace(200) %x.coerce0, i32 %x.coerce1) local_unnamed_addr addrspace(200) #1 {
...
// Here we want to check that the struct received from the callee is handled as a proper struct, i.e. fields are read using `extractvalue` rather than dereferencing pointers. Also, we want to check that arguments in composite calls are passed correctly.
// CHECK: define dso_local chericcallee void @_Z11CheckPtrIntv() local_unnamed_addr addrspace(200) #2 {
__attribute__((cheri_compartment("example"))) void CheckPtrInt() {

  static struct PointerAndInt __attribute__((used)) x = {}; 

  // CHECK: entry:
  // CHECK:   %call = tail call chericcallee %struct.PointerAndInt @_Z10InitPtrIntv()
  // CHECK:   %0 = extractvalue %struct.PointerAndInt %call, 0
  // CHECK:   %1 = extractvalue %struct.PointerAndInt %call, 1
  // CHECK:   %call1 = tail call chericcallee %struct.PointerAndInt @_Z9ChgPtrInt13PointerAndInt(ptr addrspace(200) %0, i32 %1)
...
// Here we want to check that the result is returned by value.
// CHECK: define dso_local chericcallee %struct.IntAndPointer @_Z10InitIntPtrv() local_unnamed_addr addrspace(200) #1 {
...
// Here we want to check that the struct received as parameter is laid out as two different arguments and, again, is returned by value.
// CHECK: define dso_local chericcallee %struct.IntAndPointer @_Z9ChgIntPtr13IntAndPointer(i32 %x.coerce0, ptr addrspace(200) %x.coerce1) local_unnamed_addr addrspace(200) #1 {
...
// Here we want to check that the struct received from the callee is handled as a proper struct, i.e. fields are read using `extractvalue` rather than dereferencing pointers. Also, we want to check that arguments in composite calls are passed correctly.
// CHECK: define dso_local chericcallee void @_Z11CheckIntPtrv() local_unnamed_addr addrspace(200) #2 {
__attribute__((cheri_compartment("example"))) void CheckIntPtr() {

  static struct IntAndPointer __attribute__((used)) x = {}; 

  // CHECK: entry:
  // CHECK:   %call = tail call chericcallee %struct.IntAndPointer @_Z10InitIntPtrv()
  // CHECK:   %0 = extractvalue %struct.IntAndPointer %call, 0
  // CHECK:   %1 = extractvalue %struct.IntAndPointer %call, 1
  // CHECK:   %call1 = tail call chericcallee %struct.IntAndPointer @_Z9ChgIntPtr13IntAndPointer(i32 %0, ptr addrspace(200) %1)
...
// Here we want to check that the result is returned by value.
// CHECK: define dso_local chericcallee [2 x i32] @_Z10InitParentv() local_unnamed_addr addrspace(200) #1 {
...
// Here we want to check that the struct received as parameter is laid out as two different arguments and, again, is returned by value.
// CHECK: define dso_local chericcallee [2 x i32] @_Z9ChgParent6Parent([2 x i32] %x.coerce) local_unnamed_addr addrspace(200) #1 {
...
// Here we want to check that the struct received from the callee is handled as a proper struct, i.e. fields are read using `extractvalue` rather than dereferencing pointers. Also, we want to check that arguments in composite calls are passed correctly.
// CHECK: define dso_local chericcallee void @_Z11CheckParentv() local_unnamed_addr addrspace(200) #2 {
__attribute__((cheri_compartment("example"))) void CheckParent() {

  static struct Parent __attribute__((used)) x = {}; 

  // CHECK: entry:
  // CHECK:   %call = tail call chericcallee [2 x i32] @_Z10InitParentv()
  // CHECK:   %call1 = tail call chericcallee [2 x i32] @_Z9ChgParent6Parent([2 x i32] %call)
  // CHECK:   %call1.fca.0.extract = extractvalue [2 x i32] %call1, 0
  // CHECK:   %call1.fca.1.extract = extractvalue [2 x i32] %call1, 1
...
// Here we want to check that the result is returned by value.
// CHECK: define dso_local chericcallee %struct.ParentPtr @_Z13InitParentPtrv() local_unnamed_addr addrspace(200) #1 {
...
// Here we want to check that the struct received as parameter is laid out as two different arguments and, again, is returned by value.
// CHECK: define dso_local chericcallee %struct.ParentPtr @_Z12ChgParentPtr9ParentPtr(ptr addrspace(200) %x.coerce0, %struct.InnerPtr %x.coerce1) local_unnamed_addr addrspace(200) #1 {
__attribute__((cheri_compartment("example"), noinline)) struct ParentPtr ChgParentPtr(struct ParentPtr x) {
  ...
  // CHECK:   %call3 = tail call chericcallee i32 @_Z8GetValuev()
...
// Here we want to check that the struct received from the callee is handled as a proper struct, i.e. fields are read using `extractvalue` rather than dereferencing pointers. Also, we want to check that arguments in composite calls are passed correctly.
// CHECK: define dso_local chericcallee void @_Z14CheckParentPtrv() local_unnamed_addr addrspace(200) #2 {
__attribute__((cheri_compartment("example"))) void CheckParentPtr() {

  static struct ParentPtr __attribute__((used)) x = {}; 

  // CHECK: entry:
  // CHECK:   %call = tail call chericcallee %struct.ParentPtr @_Z13InitParentPtrv()
  // CHECK:   %0 = extractvalue %struct.ParentPtr %call, 0
  // CHECK:   %1 = extractvalue %struct.ParentPtr %call, 1
  // CHECK:   %call1 = tail call chericcallee %struct.ParentPtr @_Z12ChgParentPtr9ParentPtr(ptr addrspace(200) %0, %struct.InnerPtr %1)
...
// CHECK: define dso_local chericcallee [2 x i32] @_Z8ChgInts2i11TwoIntegers(i32 noundef %new_int, [2 x i32] %x.coerce) local_unnamed_addr addrspace(200) #4 {
...
// CHECK: define dso_local chericcallee %struct.TwoPointers @_Z8ChgPtrs2i11TwoPointers(i32 noundef %new_int, ptr addrspace(200) %x.coerce0, ptr addrspace(200) %x.coerce1) local_unnamed_addr addrspace(200) #1 {
...
// CHECK: define dso_local chericcallee %struct.ParentPtr @_Z13ChgParentPtr2i9ParentPtr(i32 noundef %new_int, ptr addrspace(200) %x.coerce0, %struct.InnerPtr %x.coerce1) local_unnamed_addr addrspace(200) #1 {
...
// CHECK: define dso_local chericcallee [2 x i32] @_Z8ChgInts3i11TwoIntegersz(i32 noundef %n, [2 x i32] %x.coerce, ...) local_unnamed_addr addrspace(200) #5 {
...
// CHECK: define dso_local chericcallee %struct.TwoPointers @_Z8ChgPtrs3i11TwoPointersz(i32 noundef %n, ptr addrspace(200) %x.coerce0, ptr addrspace(200) %x.coerce1, ...) local_unnamed_addr addrspace(200) #7 {
...
// CHECK: define dso_local chericcallee %struct.ParentPtr @_Z13ChgParentPtr3i9ParentPtrz(i32 noundef %n, ptr addrspace(200) %x.coerce0, %struct.InnerPtr %x.coerce1, ...) local_unnamed_addr addrspace(200) #7 {
...
// CHECK: define dso_local chericcallee [2 x i32] @_Z8ChgInts4iiiii11TwoIntegers(i32 noundef %n0, i32 noundef %n1, i32 noundef %n2, i32 noundef %n3, i32 noundef %n4, [2 x i32] %x.coerce) local_unnamed_addr addrspace(200) #4 {
...
// CHECK: define dso_local chericcallee %struct.TwoPointers @_Z8ChgPtrs4iiiii11TwoPointers(i32 noundef %n0, i32 noundef %n1, i32 noundef %n2, i32 noundef %n3, i32 noundef %n4, ptr addrspace(200) %x.coerce0, ptr addrspace(200) %x.coerce1) local_unnamed_addr addrspace(200) #1 {
__attribute__((cheri_compartment("example"), noinline)) struct TwoPointers ChgPtrs4(int n0, int n1, int n2, int n3, int n4, struct TwoPointers x) {
  ...
  // CHECK:   %call6 = tail call chericcallee i32 @_Z8GetValuev()
...
// CHECK: define dso_local chericcallee %struct.ParentPtr @_Z13ChgParentPtr4iiiii9ParentPtr(i32 noundef %n0, i32 noundef %n1, i32 noundef %n2, i32 noundef %n3, i32 noundef %n4, ptr addrspace(200) %x.coerce0, %struct.InnerPtr %x.coerce1) local_unnamed_addr addrspace(200) #1 {
__attribute__((cheri_compartment("example"), noinline)) struct ParentPtr ChgParentPtr4(int n0, int n1, int n2, int n3, int n4, struct ParentPtr x) {
  ...
  // CHECK:   %call7 = tail call chericcallee i32 @_Z8GetValuev()
...
// CHECK: define dso_local chericcallee i32 @_Z10InitOneIntv() local_unnamed_addr addrspace(200) #1 {
...
// CHECK: define dso_local chericcallee i32 @_Z9ChgOneInt6OneInt(i32 %x.coerce) local_unnamed_addr addrspace(200) #1 {
...
// Here we want to check that the initial struct received from the callee is handled as a proper struct, i.e. fields are read using `extractvalue` rather than dereferencing pointers.
// CHECK: define dso_local chericcallee void @_Z11CheckOneIntv() local_unnamed_addr addrspace(200) #2 {
__attribute__((cheri_compartment("example"))) void CheckOneInt() {

  static struct OneInt __attribute__((used)) x = {}; 

  // CHECK: entry:
  // CHECK:   %call = tail call chericcallee i32 @_Z10InitOneIntv()
  // CHECK:   %call2 = tail call chericcallee i32 @_Z9ChgOneInt6OneInt(i32 %call)
...
// CHECK: define dso_local chericcallee %struct.OnePtr @_Z10InitOnePtrv() local_unnamed_addr addrspace(200) #1 {
...
// CHECK: define dso_local chericcallee %struct.OnePtr @_Z9ChgOnePtr6OnePtr(ptr addrspace(200) %x.coerce) local_unnamed_addr addrspace(200) #1 {
...
// CHECK: define dso_local chericcallee void @_Z11CheckOnePtrv() local_unnamed_addr addrspace(200) #2 {
__attribute__((cheri_compartment("example"))) void CheckOnePtr () {

  static struct OnePtr __attribute__((used)) x = {}; 

  // CHECK: entry:
  // CHECK:   %call = tail call chericcallee %struct.OnePtr @_Z10InitOnePtrv()
  // CHECK:   %0 = extractvalue %struct.OnePtr %call, 0
  // CHECK:   %call1 = tail call chericcallee %struct.OnePtr @_Z9ChgOnePtr6OnePtr(ptr addrspace(200) %0)
