...
// RUN: rm -f %t-debug.csv %t-nodebug.csv
...
int use_stack_var() {
  ...
  // CSV-DEBUG-NEXT:   2,4,s,"{{.*}}llvm-project/clang/test/CodeGen/cheri/csetbounds-stats-purecap-stack.c:18:7","CHERI bound stack allocations","set bounds on local variable x"
}
