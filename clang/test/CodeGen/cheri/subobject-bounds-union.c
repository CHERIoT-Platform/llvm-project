...
void test2(union WithNestedStruct* un) {
  ...
  call(&un->nested);
  // aggressive-remark-re@-1{{using size of containing type 'union WithNestedStruct' instead of object type 'struct (unnamed at {{.+}}subobject-bounds-union.c:{{[0-9]+}}:3)' for subobject bounds on union member}}
  // aggressive-remark-re@-2{{setting sub-object bounds for pointer to 'struct (unnamed at {{.+}}subobject-bounds-union.c:{{[0-9]+}}:3)' to 64 bytes}}
  // very-aggressive-remark-re@-3{{setting sub-object bounds for field 'nested' (pointer to 'struct (unnamed at {{.+}}subobject-bounds-union.c:{{[0-9]+}}:3)') to 8 bytes}}
  call(&un->buffer);
