...
// CHECK: [[CAST:%.*]] = inttoptr i64 [[RESULT]] to ptr
// CHECK: [[PHI:%.*]] = phi ptr [ null, {{%.*}} ], [ [[CAST]], {{%.*}} ]
// CHECK: store atomic ptr [[PHI]], ptr [[ADDR]] unordered
...
// CHECK: [[VALUE:%.*]] = load ptr, ptr %field3.addr, align 8
// CHECK: [[CASTED_ADDR:%.*]] = ptrtoint ptr {{%.*}} to i64
// CHECK: [[BLENDED:%.*]] = call i64 @llvm.ptrauth.blend(i64 [[CASTED_ADDR]], i64 1)
// CHECK: [[CAST_VALUE:%.*]] = ptrtoint ptr [[VALUE]] to i64
// CHECK: {{%.*}} = call i64 @llvm.ptrauth.resign(i64 [[CAST_VALUE]], i32 0, i64 0, i32 1, i64 [[BLENDED]])
// CHECK: store atomic ptr
