...
void check_bad_macro_values(void *mtx) {
  ...
  // expected-error@-1{{static assertion failed due to requirement '32 < (31 + 1)': Using too many low pointer bits}} expected-note@-1{{expression evaluates to '32 < 32'}}
  // expected-error@-2{{static assertion failed due to requirement '(32 & (32 + 1)) == 0': Mask must be all ones}} expected-note@-2{{expression evaluates to '32 == 0'}}
