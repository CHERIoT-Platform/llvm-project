// RUN: %cheri_purecap_cc1 -fsyntax-only %s -ast-dump - -verify | FileCheck %s
...
// CHECK-NEXT: | |-FieldDecl {{.*}} <line:7:2, col:8> col:8 a '__ptrdiff_t':'long'
// CHECK-NEXT: | |-FieldDecl {{.*}} <line:8:2, col:7> col:7 d 'long'
// CHECK-NEXT: | `-FieldDecl {{.*}} <line:9:2, col:7> col:7 e 'long'
// CHECK-NEXT: |-VarDecl {{.*}} <line:11:1, col:12> col:12 referenced si 'struct foo'
// CHECK-NEXT: |-VarDecl {{.*}} <line:12:1, col:7> col:7 referenced a '__ptrdiff_t':'long'
