...
bool UnwindCursor<A, R>::getInfoFromFdeCie(
  ...
  if (CFI_Parser<A>::template parseFDEInstructions<R>(
          _addressSpace, fdeInfo, cieInfo, pc.address(), R::getArch(),
                                          &prolog)) {
...
bool UnwindCursor<A, R>::getInfoFromDwarfSection(
  ...
  if (!foundFDE) {
    ...
    pint_t cachedFDE =
        DwarfFDECache<A>::template findFDE<R>(sects.dso_base, pc.address());
    if (cachedFDE != 0) {
      foundFDE = CFI_Parser<A>::template findFDE<R>(
          _addressSpace, pc, sects.dwarf_section(), sects.dwarf_section_length,
...
void UnwindCursor<A, R>::setInfoBasedOnIPRegister(bool isReturnAddress) {
  ...
  pint_t cachedFDE =
      DwarfFDECache<A>::template findFDE<R>(DwarfFDECache<A>::kSearchAll, pc.address());
  if (cachedFDE != 0) {
    typename CFI_Parser<A>::FDE_Info fdeInfo;
    typename CFI_Parser<A>::CIE_Info cieInfo;
  ...
  // other FDEs.
  pint_t fde;
  if (_addressSpace.template findOtherFDE<R>(pc.address(), fde)) {
    typename CFI_Parser<A>::FDE_Info fdeInfo;
    typename CFI_Parser<A>::CIE_Info cieInfo;
