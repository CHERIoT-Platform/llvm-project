...
void LoongArch::scanSectionImpl(InputSectionBase &sec, Relocs<RelTy> rels) {
  ...
  for (auto it = rels.begin(); it != rels.end(); ++it) {
    ...
    case R_LARCH_TLS_GD_PC_HI20:
      ...
      sec.addReloc(ctx, {RE_LOONGARCH_TLSGD_PAGE_PC, type, offset, addend, &sym});
      continue;
    case R_LARCH_TLS_LD_HI20:
      ctx.needsTlsLd.store(true, std::memory_order_relaxed);
      sec.addReloc(ctx, {R_TLSLD_GOT, type, offset, addend, &sym});
      continue;
    case R_LARCH_TLS_GD_HI20:
      sym.setFlags(NEEDS_TLSGD);
      sec.addReloc(ctx, {R_TLSGD_GOT, type, offset, addend, &sym});
      continue;
    case R_LARCH_TLS_LD_PCREL20_S2:
    case R_LARCH_TLS_LD_PCADD_HI20:
      ctx.needsTlsLd.store(true, std::memory_order_relaxed);
      sec.addReloc(ctx, {R_TLSLD_PC, type, offset, addend, &sym});
      continue;
    case R_LARCH_TLS_GD_PCREL20_S2:
    case R_LARCH_TLS_GD_PCADD_HI20:
      sym.setFlags(NEEDS_TLSGD);
      sec.addReloc(ctx, {R_TLSGD_PC, type, offset, addend, &sym});
      continue;
    ...
    case R_LARCH_TLS_DESC64_PC_HI12:
      ...
      sec.addReloc(ctx, {RE_LOONGARCH_TLSDESC_PAGE_PC, type, offset, addend, &sym});
      continue;
    ...
    case R_LARCH_TLS_DESC_PCADD_HI20:
      sym.setFlags(NEEDS_TLSDESC);
      sec.addReloc(ctx, {R_TLSDESC, type, offset, addend, &sym});
      continue;
