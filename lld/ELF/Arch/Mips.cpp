...
uint64_t MIPS<ELFT>::getCheriRequiredAlignment(uint64_t len) const {
  if ((ctx.arg.eflags & EF_MIPS_MACH) == EF_MIPS_MACH_CHERI128)
    return llvm::RV64YCapabilityFormat::getRequiredAlignment(len)
        .value();
  return 1;
}
...
void MIPS<ELFT>::scanSectionImpl(InputSectionBase &sec, Relocs<RelTy> rels) {
  ...
  for (auto it = rels.begin(); it != rels.end();) {
    ...
    if (expr == RE_MIPS_TLSLD) {
      ...
      sec.addReloc(ctx, {expr, type, offset, addend, &sym});
    } else if (expr == RE_MIPS_CHERI_CAPTAB_TLSLD) {
      ctx.in.mipsCheriCapTable->addTlsIndex();
      sec.addReloc(ctx, {expr, type, offset, addend, &sym});
    } else if (expr == RE_MIPS_CHERI_CAPTAB_TLSGD) {
      ctx.in.mipsCheriCapTable->addDynTlsEntry(sym);
      sec.addReloc(ctx, {expr, type, offset, addend, &sym});
    } else if (expr == RE_MIPS_CHERI_CAPTAB_TPREL) {
