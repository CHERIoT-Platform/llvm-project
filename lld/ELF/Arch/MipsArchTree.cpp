...
void elf::checkMipsShlibCompatible(Ctx &ctx, InputFile *f, uint64_t inputCheriFlags,
                                   uint64_t targetCheriFlags) {
  const uint32_t targetABI = ctx.arg.eflags & (EF_MIPS_ABI | EF_MIPS_ABI2);
  ...
  if ((ctx.arg.isCheriAbi && abi != EF_MIPS_ABI_CHERIABI) ||
      (!ctx.arg.isCheriAbi && abi == EF_MIPS_ABI_CHERIABI)) {
    // assert(errorCount() && "Should have already caused an errors");
    // llvm_unreachable("Should have been checked earlier!");
    if (!errorCount())
      error(toStr(ctx, f) + ": ABI '" + getAbiName(abi) +
