  ...
  for (auto it = rels.begin(); it != rels.end(); ++it) {
    ...
    case R_PPC64_TLSLD: {
      ...
      if (it1->getType(false) == R_PPC64_REL24_NOTOC)
        ++offset;
      if (optimizeTlsGdLd) {
        sec.addReloc(ctx, {sym.isPreemptible ? R_GOT_OFF : R_TPREL, type, offset,
                      addend, &sym});
        ++it; // skip REL24
      }
      continue;
    ...
    case R_PPC64_DTPREL34:
      sec.addReloc(ctx, {R_DTPREL, type, offset, addend, &sym});
      continue;
    ...
    case R_PPC64_GOT_DTPREL16_HI:
      ...
      sec.addReloc(ctx, {R_TLSLD_GOT_OFF, type, offset, addend, &sym});
      continue;
