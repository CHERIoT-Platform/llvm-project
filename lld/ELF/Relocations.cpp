...
void elf::printLocation(ELFSyncStream &s, const InputSectionBase &sec,
...
static void addRelativeReloc(Ctx &ctx, InputSectionBase &isec,
  ...
  if (expr == R_ABS_CAP && !ctx.arg.useRelativeElfCheriRelocs) {
    auto fn = [&]() {
      ctx.in.capRelocs->addReloc(isec, offsetInSec, sym, addend, expr, type);
    };
  ...
  if (sym.isTagged() &&
      (addend < 0 || static_cast<uint64_t>(addend) >= sym.getSize(ctx)))
    isec.addReloc(ctx, {R_ADDEND_NEG, type, offsetInSec, addend, &sym});
}
...
static void addPltEntry(Ctx &ctx, PltSection &plt, GotPltSection &gotPlt,
  ...
  if (ctx.arg.isCheriAbi && !ctx.arg.useRelativeElfCheriRelocs) {
    if (!sym.isPreemptible) {
      ctx.in.capRelocs->addReloc(gotPlt, sym.getGotPltOffset(ctx), sym,
                                        0, R_ABS_CAP,
                                        *ctx.target->symbolicCapRel);
      return;
    }

    ctx.in.capRelocs->addReloc(gotPlt, sym.getGotPltOffset(ctx), plt, 0,
                                      R_ABS_CAP,
...
static void addTgotEntry(Ctx &ctx, Symbol &sym) {
  ...
  if (sym.isUndefWeak())
    ...
    ctx.in.relaTgot->addReloc(false,
...
bool RelocScan::isStaticLinkTimeConstant(RelExpr e, RelType type,
  ...
  if (oneof<R_GOTPLT, R_GOT_OFF, R_RELAX_HINT, RE_MIPS_GOT_LOCAL_PAGE,
            ...
            RE_MIPS_CHERI_CAPTAB_REL, RE_CHERIOT1_COMPARTMENT_CGPREL_HI,
            RE_CHERIOT1_COMPARTMENT_CGPREL_LO, RE_CHERIOT1_COMPARTMENT_SIZE>(e))
    return true;
...
void elf::postScanRelocations(Ctx &ctx) {
  ...
  auto fn = [&](Symbol &sym) {
    ...
    if (flags & NEEDS_TLSGD) {
      ...
      if (sym.isPreemptible) {
        ...
        if (ctx.arg.shared)
          ...
          got->addConstant(ctx, {R_ADDEND, ctx.target->symbolicRel, off, 1, &sym});
    ...
    if (flags & NEEDS_TGOT_TLSDESC) {
      ...
      ctx.in.relaDyn->addReloc(
          false, ctx.target->tgotTlsDescRel,
