...
static bool isRelExpr(RelExpr expr) {
  return oneof<R_PC, R_GOTREL, R_GOTPLTREL, RE_ARM_PCA, RE_MIPS_GOTREL,
               ...
               RE_LOONGARCH_PAGE_PC, R_CHERI_CAPABILITY_TABLE_REL>(expr);
}
...
void RelocationScanner::processAux(RelExpr expr, RelType type, uint64_t offset,
  ...
  if (isStaticLinkTimeConstant(expr, type, sym, offset) ||
      (!ctx.arg.isPic && sym.isUndefWeak() && expr != R_CHERI_CAPABILITY)) {
  ...
  if (expr == R_CHERI_CAPABILITY) {
    ...
    addCapabilityRelocation<ELFT>(ctx, &sym, type, sec, offset, expr, addend,
                                  /* isCallExpr=*/false,
