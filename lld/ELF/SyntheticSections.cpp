      ...
      if (s == nullptr) {
        ...
        if (s->isPreemptible) {
          ...
          if (ctx.arg.shared)
            ...
            addConstant(ctx, {R_ADDEND, ctx.target->symbolicRel, off, 1, s});
          addConstant(ctx, {R_ABS, ctx.target->tlsOffsetRel, offsetOff, 0, s});
        }
      }
    }
...
RelocationBaseSection::RelocationBaseSection(Ctx &ctx, StringRef name,
    ...
    : SyntheticSection(ctx, name, type, SHF_ALLOC, ctx.arg.wordsize),
      ...
      relativeFuncRel(ctx.target->relativeFuncRel),
      combreloc(combreloc) {}
