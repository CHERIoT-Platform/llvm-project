...
protected:
  void computeRels();
  void maybeAddRISCVendorRelocation(const DynamicReloc &reloc, SmallVector<DynamicReloc, 0> &outRelocs);
...
inline void RelocationBaseSection::addReloc<true>(const DynamicReloc &reloc) {
  maybeAddRISCVendorRelocation(reloc, relocsVec[llvm::parallel::getThreadIndex()]);
