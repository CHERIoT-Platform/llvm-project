...
; HEADERS-LABEL: There are 9 program headers, starting at
; HEADERS-EMPTY:
...
; HEADERS-NEXT: 03     .gcc_except_table .data .captable .got .pad.cheri.pcc
; HEADERS-NEXT: 04     .dynamic
; HEADERS-NEXT: 05     __cap_relocs .MIPS.abiflags .MIPS.options .dynsym .hash .dynamic .dynstr .rel.dyn .rel.plt .eh_frame .text .gcc_except_table .data .captable .got .pad.cheri.pcc
; HEADERS-NEXT: 06
; HEADERS-NEXT: 07     .MIPS.options
; HEADERS-NEXT: 08     .MIPS.abiflags
...
; RV64-RELOCS-NEXT:  {{0*}}{{.+}}     9000000000000000 CODE    [[#TEST_ADDR+92]] [{{[0-9a-f]+}}-{{[0-9a-f]+}}]
; RV64-RELOCS-NEXT:  {{0*}}{{.+}}     9000000000000000 CODE    [[#TEST_ADDR+72]] [{{[0-9a-f]+}}-{{[0-9a-f]+}}]
; RV64-RELOCS-NEXT:  {{0*}}{{.+}}     9000000000000000 CODE    [[#TEST2_ADDR+68]] [{{[0-9a-f]+}}-{{[0-9a-f]+}}]
; Next one references the local symbol, and uses that length rather than the override:
; RV64-RELOCS-NEXT:  {{0*}}{{.+}}     9000000000000000 CODE    [[#TEST_WEAK_ADDR+28]] [{{[0-9a-f]+}}-{{[0-9a-f]+}}]
; RV64-RELOCS-NEXT:  {{0*}}{{.+}}     9000000000000000 CODE    [[#PLT0_ADDR]] [{{[0-9a-f]+}}-{{[0-9a-f]+}}]
; RV64-RELOCS-NEXT:  {{0*}}{{.+}}     9000000000000000 CODE    [[#PLT0_ADDR]] [{{[0-9a-f]+}}-{{[0-9a-f]+}}]
; RV64-RELOCS-NEXT:  {{0*}}{{.+}}     9000000000000000 CODE    [[#PLT0_ADDR]] [{{[0-9a-f]+}}-{{[0-9a-f]+}}]
