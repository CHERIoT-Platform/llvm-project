...
SDValue SelectionDAG::getExternalFunctionSymbol(RTLIB::LibcallImpl Libcall) {
  auto AddrSpace = getDataLayout().getProgramAddressSpace();
  StringRef SymName = TLI->getLibcallImplName(Libcall);
  return getExternalSymbol(SymName.data(), TLI->getPointerTy(getDataLayout(), AddrSpace));
}
...
getMemcpyLoadsAndStores(SelectionDAG &DAG, const SDLoc &dl, SDValue Chain,
  ...
  const bool FoundLowering = TLI.findOptimalMemOpLowering(
      C, MemOps, Limit, Op, DstPtrInfo.getAddrSpace(), SrcPtrInfo.getAddrSpace(),
  ...
  if (DL.hasCheriCapabilities() && CapTy.isValid() && !Op.isMemset()) {
    ...
    if (PreserveTags == PreserveCheriTags::Required && !ReachedLimit &&
      ...
      diagnoseInefficientCheriMemOp(
          ...
          std::max((uint64_t)1, std::min(DstAlign, SrcAlign).value()), Size,
          CapSize);
      return SDValue();
    }
  }
  ...
  if (DstAlignCanChange) {
    ...
    if (MemOps[0].isFatPointer()) {
      assert(NewDstAlign <= *DL.getStackAlignment() &&
    ...
    if (MemOps[0].isFatPointer()) {
      assert(NewDstAlign == DL.getABITypeAlign(Ty) &&
             "Stack not capability-aligned?");
    }
...
static SDValue getMemmoveLoadsAndStores(
    ...
    MachinePointerInfo DstPtrInfo,
    MachinePointerInfo SrcPtrInfo, const AAMDNodes &AAInfo,
    StringRef MoveTy, CodeGenOptLevel OptLevel) {
  ...
  if (DL.hasCheriCapabilities() && CapTy.isValid()) {
    ...
    if (PreserveTags == PreserveCheriTags::Required && !ReachedLimit &&
      ...
      diagnoseInefficientCheriMemOp(
          ...
          std::max(Align(1), std::min(DstAlign, SrcAlign)).value(), Size,
          CapSize);
      return SDValue();
    }
  }
...
SDValue SelectionDAG::getMemcpy(
  ...
  if (ConstantSize) {
    ...
    SDValue Result = getMemcpyLoadsAndStores(
        *this, dl, Chain, Dst, Src, ConstantSize->getZExtValue(), DstAlign,
        SrcAlign, isVol, false, PreserveTags, DstPtrInfo, SrcPtrInfo, AAInfo, BatchAA,
        CopyType, OptLevel);
    if (Result.getNode())
      return Result;
  }
  ...
  if (TSI) {
    SDValue Result = TSI->EmitTargetCodeForMemcpy(
        *this, dl, Chain, Dst, Src, Size, DstAlign, SrcAlign, isVol,
        AlwaysInline, PreserveTags, DstPtrInfo, SrcPtrInfo);
    if (Result.getNode())
      return Result;
  }
  ...
  if (AlwaysInline) {
    ...
    return getMemcpyLoadsAndStores(*this, dl, Chain, Dst, Src,
                                   ConstantSize->getZExtValue(), DstAlign,
                                   SrcAlign, isVol, true, PreserveTags, DstPtrInfo,
  ...
  // Emit a library call.
  TargetLowering::ArgListTy Args;
  Args.emplace_back(Dst, Dst.getValueType().getTypeForEVT(*getContext()));
  Args.emplace_back(Src, Src.getValueType().getTypeForEVT(*getContext()));
...
SDValue SelectionDAG::getMemmove(SDValue Chain, const SDLoc &dl, SDValue Dst,
  ...
  if (ConstantSize) {
    ...
    SDValue Result = getMemmoveLoadsAndStores(
        *this, dl, Chain, Dst, Src, ConstantSize->getZExtValue(), DstAlign,
        SrcAlign, isVol, false, PreserveTags, DstPtrInfo, SrcPtrInfo, AAInfo, MoveType,
        OptLevel);
    if (Result.getNode())
      return Result;
  }
  ...
  if (TSI) {
    SDValue Result = TSI->EmitTargetCodeForMemmove(
        *this, dl, Chain, Dst, Src, Size, DstAlign, SrcAlign, isVol, PreserveTags,
...
SDValue SelectionDAG::getMemset(SDValue Chain, const SDLoc &dl, SDValue Dst,
  ...
  if (UseBZero) {
    ...
    CLI.setLibCallee(
        ...
        getExternalFunctionSymbol(BzeroImpl),
        std::move(Args));
  } else {
...
SDValue SelectionDAG::makeStateFunctionCall(unsigned LibFunc, SDValue Ptr,
  ...
  SDValue Callee = getExternalFunctionSymbol(LibcallImpl);
  TargetLowering::CallLoweringInfo CLI(*this);
