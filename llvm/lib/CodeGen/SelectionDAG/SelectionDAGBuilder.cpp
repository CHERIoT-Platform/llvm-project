...
void SelectionDAGBuilder::visitRet(const ReturnInst &I) {
  ...
  if (TLI.supportSwiftError() &&
    ...
    ISD::ArgFlagsTy Flags = ISD::ArgFlagsTy();
    Flags.setSwiftError();
    Outs.push_back(ISD::OutputArg(Flags, /*vt=*/TLI.getPointerRangeTy(DL),
                                  /*argvt=*/EVT(TLI.getPointerRangeTy(DL)),
...
void SelectionDAGBuilder::visitMaskedScatter(const CallInst &I) {
  ...
  if (!UniformBase) {
    ...
    Index = getValue(Ptr);
    Scale =
        DAG.getTargetConstant(1, sdl, TLI.getPointerRangeTy(DAG.getDataLayout()));
  }
...
bool SelectionDAGBuilder::visitMemPCpyCall(const CallInst &I) {
  ...
  SDValue MC = DAG.getMemcpy(
      ...
      /*CI=*/nullptr, std::nullopt, shouldPreserveTags(&I),
      MachinePointerInfo(I.getArgOperand(0)),
    ...
    case InlineAsm::isLabel: {
      ...
      if (OpInfo.isMatchingInputConstraint()) {
        ...
        Info.AsmNodeOperands.push_back(DAG.getTargetConstant(
            Flag, DL, TLI.getPointerRangeTy(DAG.getDataLayout())));
      ...
      if (OpInfo.ConstraintType == TargetLowering::C_Immediate ||
        ...
        Info.AsmNodeOperands.push_back(DAG.getTargetConstant(
            ResOpType, DL, TLI.getPointerRangeTy(DAG.getDataLayout())));
  ...
  if (supportSwiftError()) {
    for (const ArgListEntry &Arg : Args) {
      if (Arg.IsSwiftError) {
        ISD::ArgFlagsTy Flags;
        Flags.setSwiftError();
        ISD::InputArg Ret(Flags, getPointerRangeTy(DL), EVT(getPointerRangeTy(DL)),
