...
bool AVRDAGToDAGISel::selectAlignedFrameLoad(SDNode *N) {
  ...
  MVT PointerTy = getTargetLowering()->getPointerTy(CurDAG->getDataLayout(), 0);
...
template <> bool AVRDAGToDAGISel::select<ISD::FrameIndex>(SDNode *N) {
  auto DL = CurDAG->getDataLayout();
  auto PointerTy = getTargetLowering()->getPointerTy(DL, 0);

  int FI = cast<FrameIndexSDNode>(N)->getIndex();
  SDValue TFI = CurDAG->getTargetFrameIndex(FI, getTargetLowering()->getPointerTy(DL, DL.getProgramAddressSpace()));
