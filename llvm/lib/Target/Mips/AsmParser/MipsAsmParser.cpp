...
public:
  ...
  bool isScaledSImm() const {
    ...
    if (auto Expr = dyn_cast<MipsMCExpr>(getImm())) {
      ...
      return (Kind == MipsMCExpr::MEK_CAPTABLE11) ||
             (Kind == MipsMCExpr::MEK_CAPCALL11) ||
             (Kind == MipsMCExpr::MEK_CAPTABLE20) ||
             (Kind == MipsMCExpr::MEK_CAPCALL20);
    }
...
bool MipsAsmParser::expandLoadImm(MCInst &Inst, bool Is32BitImm, SMLoc IDLoc,
  ...
  if (loadImmediate(Imm, DstRegOp.getReg(), MCRegister(), Is32BitImm,
                    false, IDLoc, Out, STI))
    return true;
...
bool MipsAsmParser::matchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  case Match_UImm11_Lsl0:
    return Error(RefineErrorLoc(IDLoc, Operands, ErrorInfo),
                 "expected 11-bit unsigned immediate");
  case Match_SImm10_0:
    return Error(RefineErrorLoc(IDLoc, Operands, ErrorInfo),
...
int MipsAsmParser::matchCheriHWRegsRegisterName(StringRef Name) {
  ...
  int Result = -1;
  for (unsigned Reg = Mips::CAPHWR0; Reg <= Mips::CAPHWR31; Reg++) {
...
ParseStatus MipsAsmParser::parseMemOperand(OperandVector &Operands) {
  ...
  if (HadLParen && Parser.getTok().isNot(AsmToken::RParen))
     return ErrorIfNotPending(Parser.getTok().getLoc(), "')' expected");
