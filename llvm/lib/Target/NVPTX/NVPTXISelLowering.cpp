...
std::string NVPTXTargetLowering::getPrototype(
  ...
  auto PtrVT = getPointerTy(DL, DL.getAllocaAddrSpace());
...
SDValue NVPTXTargetLowering::LowerVAARG(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue VAListLoad =
      DAG.getLoad(TLI->getPointerTy(DAG.getDataLayout(),
...
SDValue NVPTXTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
  const TargetLowering *TLI = STI.getTargetLowering();
  SDLoc DL(Op);
  EVT PtrVT = TLI->getPointerTy(DAG.getDataLayout(), DAG.getDataLayout().getAllocaAddrSpace());
