...
Expected<ABI> computeTargetABI(const MCSubtargetInfo &STI, StringRef ABIName) {
  ...
  if ((ABIName.starts_with("ilp32") || ABIName.starts_with("il32")) && IsRV64) {
    return createStringError(
        "32-bit ABIs are not supported for 64-bit targets");
  }
  if ((ABIName.starts_with("lp64") || ABIName.starts_with("l64")) && !IsRV64) {
    return createStringError(
        "64-bit ABIs are not supported for 32-bit targets");
  }
  if ((ABIName.starts_with("il32pc") || ABIName.starts_with("l64pc")) &&
             !FeatureBits[RISCV::FeatureVendorXCheri]) {
    return createStringError("pure-capability ABI can't be used for a target that "
              "doesn't support the XCheri instruction set extension");
  }
  ...
  if (!IsRV64 && IsRVE && !IsXCheriot && TargetABI != ABI_ILP32E &&
    ...
    return createStringError("only the ilp32e and il32pc64e ABIs are supported for RV32E");
  }
  if (!IsRV64 && IsRVE && IsXCheriot && TargetABI != ABI_CHERIOT &&
      TargetABI != ABI_CHERIOT_BAREMETAL && TargetABI != ABI_Unknown) {
    return createStringError("only the cheriot and cheriot-baremetal ABIs are supported for XCheriot");
  }
...
void validate(const Triple &TT, const FeatureBitset &FeatureBits) {
  ...
  if (FeatureBits[RISCV::FeatureVendorXCheriot]) {
    if (!FeatureBits[RISCV::FeatureVendorXCheri])
      reportFatalUsageError("XCheriot extension requires XCheri extension");
    if (!FeatureBits[RISCV::FeatureVendorXCheriPureCap])
      reportFatalUsageError("XCheriot extension requires XCheriPureCap");
  }
}
