...
static bool CC_RISCVAssign2XLen(CCState &State, bool IsPureCapVarArgs,
                                CCValAssign VA1,
...
static bool CC_RISCV_Impl(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  if (LocVT == MVT::f64 && AllowFPRForF64 && !IsPureCapVarArgs) {
  ...
  // Also handle for pure capability varargs which are always passed on the stack.
  if (XLen == 32 &&
  ...
  if ((LocVT.isScalarInteger() ||
    ...
    return CC_RISCVAssign2XLen(State, IsPureCapVarArgs, VA, AF, ValNo, ValVT, LocVT, ArgFlags,
                               Subtarget);
  }
  ...
  // who should go first?
  else if (!Subtarget.isPExtPackedType(LocVT) &&
    ...
    for (auto &It : PendingLocs) {
      if (Reg) {
        State.addLoc(CCValAssign::getReg(
            It.getValNo(), It.getValVT(), Reg,
            RISCVABI::isCheriPureCapABI(ABI) ? CLenVT : XLenVT,
