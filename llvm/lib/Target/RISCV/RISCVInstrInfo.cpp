...
std::optional<unsigned> getFoldedOpcode(MachineFunction &MF, MachineInstr &MI,
  ...
  bool Purecap = RISCVABI::isCheriPureCapABI(ST.getTargetABI());
  switch (MI.getOpcode()) {
  default:
    if (RISCVInstrInfo::isSEXT_W(MI))
      return Purecap ? RISCV::CLW : RISCV::LW;
    if (RISCVInstrInfo::isZEXT_W(MI))
      return Purecap ? RISCV::CLWU : RISCV::LWU;
    if (RISCVInstrInfo::isZEXT_B(MI))
      return Purecap ? RISCV::CLBU : RISCV::LBU;
    break;
  case RISCV::SEXT_H:
    return Purecap ? RISCV::CLH : RISCV::LH;
  case RISCV::SEXT_B:
    return Purecap ? RISCV::CLB : RISCV::LB;
  case RISCV::ZEXT_H_RV32:
  case RISCV::ZEXT_H_RV64:
    return Purecap ? RISCV::CLHU : RISCV::LHU;
  }
...
void RISCVInstrInfo::insertIndirectBranch(MachineBasicBlock &MBB,
  ...
  if (TmpGPR.isValid())
    ...
    if (RISCVABI::isCheriPureCapABI(ST.getTargetABI())) {
      TmpGPR = STI.hasStdExtE() ? RISCV::X9_Y : RISCV::X27_Y;
...
RISCVInstrInfo::getSerializableDirectMachineOperandTargetFlags() const {
  ...
  static const std::pair<unsigned, const char *> TargetFlags[] = {
      ...
      {MO_CHERIOT1_COMPARTMENT_CODE_HI, "riscv-cheriot-compartment-code-hi"},
      {MO_CHERIOT1_COMPARTMENT_DATA_HI, "riscv-cheriot-compartment-data-hi"},
...
MachineBasicBlock::iterator RISCVInstrInfo::insertOutlinedCall(
  ...
  if (C.CallConstructionID == MachineOutlinerRegSave) {
    ...
    It = MBB.insert(It, BuildMI(MF, DebugLoc(),
                                get(IsPurecap ? RISCV::PseudoCCALLReg
                                              : RISCV::PseudoCALLReg),
