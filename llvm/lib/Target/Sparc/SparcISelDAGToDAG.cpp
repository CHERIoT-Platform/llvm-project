...
bool SparcDAGToDAGISel::SelectADDRri(SDValue Addr,
  ...
  if (FrameIndexSDNode *FIN = dyn_cast<FrameIndexSDNode>(Addr)) {
    Base = CurDAG->getTargetFrameIndex(
        FIN->getIndex(), TLI->getPointerTy(CurDAG->getDataLayout(), 0));
...
bool SparcDAGToDAGISel::SelectADDRrr(SDValue Addr, SDValue &R1, SDValue &R2) {
  ...
  R1 = Addr;
  R2 = CurDAG->getRegister(SP::G0, TLI->getPointerTy(CurDAG->getDataLayout(), 0));
  return true;
}
...
bool SparcDAGToDAGISel::SelectForceADDRrr(SDValue Addr, SDValue &Base,
  ...
  Base = Addr;
  Disp =
      CurDAG->getRegister(SP::G0, TLI->getPointerTy(CurDAG->getDataLayout(), 0));
  return true;
}
