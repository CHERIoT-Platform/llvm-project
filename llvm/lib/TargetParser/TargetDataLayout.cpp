...
namespace {
enum class MipsABI { Unknown, O32, N32, N64, N64_PureCap };
}
...
static std::string computeMipsDataLayout(const Triple &TT, StringRef ABIName, StringRef FS) {
  std::string Ret;
  ...
  if (ABI != MipsABI::N64 && ABI != MipsABI::N64_PureCap)
    Ret += "-p:32:32";
  ...
  // 128 bit aligned.
  if (ABI == MipsABI::N64 || ABI == MipsABI::N32 || ABI == MipsABI::N64_PureCap)
    Ret +=
        "-i128:128-n32:64-S" + llvm::utostr(std::max(128u, MinStackAlignBits));
  else
    Ret += "-n32-S64";

  // TODO: we may want to put functions in AS201 at some point
  if (ABI == MipsABI::N64_PureCap)
    Ret += "-A200-P200-G200";
...
std::string Triple::computeDataLayout(StringRef ABIName, StringRef FS) const {
  ...
  case Triple::mips64el:
    return computeMipsDataLayout(*this, ABIName, FS);
  case Triple::msp430:
