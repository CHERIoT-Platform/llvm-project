...
; RUN: llc -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+xcheri,+xcheripurecap,+f --relocation-model=pic < %s -o - | FileCheck %s
; RUN: llc -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+xcheri,+xcheripurecap,+f --relocation-model=pic < %s -o - -filetype=obj | llvm-readobj --relocs --symbols - | FileCheck %s --check-prefix=RELOCS
...
; CHECK-NEXT:    csc cs0, 0(csp) # 8-byte Folded Spill
...
; CHECK-NEXT:    clc cs0, 0(csp) # 8-byte Folded Reload
