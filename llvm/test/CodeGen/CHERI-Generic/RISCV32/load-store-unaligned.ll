...
; DBG-NEXT: warning: <unknown>:0:0: in function store_unaligned void (ptr addrspace(200), ptr addrspace(200), ptr addrspace(200)): found underaligned store of capability type (aligned to 4 bytes instead of 8). Will use memcpy() instead of capability load to preserve tags if it is aligned correctly at runtime
...
; DBG-NEXT: 3,8,s,"<somewhere in store_unaligned>","expanding unaligned capability load/store","expanding unaligned capability store stack source"
; DBG-NEXT: 3,8,s,"<somewhere in store_unaligned>","expanding unaligned capability load/store","expanding unaligned capability store memcpy destination"
