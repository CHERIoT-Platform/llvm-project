...
; RUN: llc -mtriple=riscv32 --relocation-model=pic -target-abi il32pc64f -mattr=+xcheri,+xcheripurecap,+f -relocation-model=static < %s -o - | FileCheck %s
...
; CHECK-NEXT:    auipcc a2, %pcrel_hi(.LJTI2_0)
; CHECK-NEXT:    cincoffset a2, a2, %pcrel_lo(.LBB2_10)
; CHECK-NEXT:    cincoffset a0, a2, a0
; CHECK-NEXT:    clw a0, 0(a0)
; CHECK-NEXT:  .LBB2_11: # %entry
; CHECK-NEXT:    # Label of block must be emitted
; CHECK-NEXT:    auipcc a2, %pcrel_hi(.Labove_threshold_all$jump_table_base)
; CHECK-NEXT:    cincoffset a2, a2, %pcrel_lo(.LBB2_11)
; CHECK-NEXT:    cincoffset a0, a2, a0
; CHECK-NEXT:    cjr a0
