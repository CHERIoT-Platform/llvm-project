...
; RUN: opt -data-layout="e-m:e-pf200:128:128:128:64-p:64:64-i64:64-i128:128-n64-S128-A200-P200-G200" -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d "-passes=default<O3>" -S < %s | FileCheck %s --check-prefix=HOIST-OPT
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d -O3 < %s | FileCheck %s
...
; CHECK:       # %bb.0: # %entry
; CHECK-NEXT:    cincoffset sp, sp, -96
; CHECK-NEXT:    csc ra, 80(sp) # 16-byte Folded Spill
; CHECK-NEXT:    csc s0, 64(sp) # 16-byte Folded Spill
; CHECK-NEXT:    csc s1, 48(sp) # 16-byte Folded Spill
; CHECK-NEXT:    csc s2, 32(sp) # 16-byte Folded Spill
; CHECK-NEXT:    csc s3, 16(sp) # 16-byte Folded Spill
; CHECK-NEXT:    csc s4, 0(sp) # 16-byte Folded Spill
...
; CHECK-NEXT:    clc ra, 80(sp) # 16-byte Folded Reload
; CHECK-NEXT:    clc s0, 64(sp) # 16-byte Folded Reload
; CHECK-NEXT:    clc s1, 48(sp) # 16-byte Folded Reload
; CHECK-NEXT:    clc s2, 32(sp) # 16-byte Folded Reload
; CHECK-NEXT:    clc s3, 16(sp) # 16-byte Folded Reload
; CHECK-NEXT:    clc s4, 0(sp) # 16-byte Folded Reload
; CHECK-NEXT:    cincoffset sp, sp, 96
; CHECK-NEXT:    cret
...
; HOIST-OPT-NEXT:    br i1 [[CMP]], label [[FOR_BODY]], label [[FOR_COND_CLEANUP_SPLIT]]
;
entry:
...
if.then:                                          ; preds = %for.body
  %address.with.bounds = call addrspace(200) ptr addrspace(200) @llvm.cheri.cap.bounds.set.i64(ptr addrspace(200) nonnull %f, i64 4)
  %address.with.bounds1 = call addrspace(200) ptr addrspace(200) @llvm.cheri.cap.bounds.set.i64(ptr addrspace(200) nonnull %dst, i64 4)
  call addrspace(200) void @call(ptr addrspace(200) %address.with.bounds, ptr addrspace(200) %address.with.bounds1)
  br label %for.inc
