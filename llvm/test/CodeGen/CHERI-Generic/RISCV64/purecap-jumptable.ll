...
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d -relocation-model=static < %s -o - | FileCheck %s
...
; CHECK-NEXT:    csc ra, 0(sp) # 16-byte Folded Spill
...
; CHECK-NEXT:  .LBB2_8:
; CHECK-NEXT:    csw a0, 0(a1)
; CHECK-NEXT:    clc ra, 0(sp) # 16-byte Folded Reload
; CHECK-NEXT:    cincoffset sp, sp, 16
