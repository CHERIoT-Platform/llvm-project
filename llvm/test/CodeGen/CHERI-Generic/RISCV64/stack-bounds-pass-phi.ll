...
; RUN: opt -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d -passes=cheri-bound-allocas %s -o - -S \
; RUN:    -cheri-stack-bounds-single-intrinsic-threshold=10 | FileCheck %s
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+xcheri,+xcheripurecap,+f,+d -O2 -cheri-stack-bounds-single-intrinsic-threshold=10 < %s | %cheri_FileCheck %s -check-prefix ASM
...
; ASM-NEXT:    csw a2, 8(sp)
; ASM-NEXT:    li a1, 3
; ASM-NEXT:    cincoffset a2, sp, 8
