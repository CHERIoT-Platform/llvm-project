# RUN: llvm-mc %s -triple=riscv32 -mattr=+c,+xcheri,+xcheripurecap -riscv-no-aliases -show-encoding \
# RUN:     | FileCheck -check-prefixes=CHECK-ASM,CHECK-ASM-AND-OBJ %s
# RUN: llvm-mc %s -triple=riscv64 -mattr=+c,+xcheri,+xcheripurecap -riscv-no-aliases -show-encoding \
# RUN:     | FileCheck --check-prefixes=CHECK-ASM,CHECK-ASM-AND-OBJ %s
# RUN: llvm-mc -filetype=obj -triple riscv32 -mattr=+c,+xcheri,+xcheripurecap < %s \
# RUN:     | llvm-objdump --no-print-imm-hex -M no-aliases --mattr=+c,+xcheri,+xcheripurecap -d - \
# RUN:     | FileCheck -check-prefixes=CHECK-ASM-AND-OBJ %s
# RUN: llvm-mc -filetype=obj -triple riscv64 -mattr=+c,+xcheri,+xcheripurecap < %s \
# RUN:     | llvm-objdump --no-print-imm-hex -M no-aliases --mattr=+c,+xcheri,+xcheripurecap -d - \
# RUN:     | FileCheck --check-prefixes=CHECK-ASM-AND-OBJ %s
