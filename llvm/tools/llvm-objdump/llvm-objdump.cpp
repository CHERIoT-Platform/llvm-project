...
DisassemblerTarget::DisassemblerTarget(const Target *TheTarget, ObjectFile &Obj,
    ...
    : TheTarget(TheTarget), TheTriple(TripleName),
      ...
      RegisterInfo(TheTarget->createMCRegInfo(TheTriple, Options)) {
  if (!RegisterInfo)
      ...
      while (Index < End) {
        ...
        if (DumpARMELFData) {
          ...
          if (Disassembled && DT->InstrAnalysis) {
            ...
            if (DT->InstrAnalysis && CheriCapTableAddress &&
                DT->InstrAnalysis->isCapTableLoad(Inst, CapTableOffset) &&
...
int llvm_objdump_main(int argc, char **argv, const llvm::ToolContext &) {
  ...
  const bool ShouldDump =
      ArchiveHeaders || Disassemble || DwarfDumpType != DIDT_Null || CapRelocations ||
