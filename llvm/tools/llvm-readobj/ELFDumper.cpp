...
constexpr EnumStringDef<unsigned> ElfDynamicDTMipsCheriAbiFlagsDefs[] = {
...
constexpr auto ElfDynamicDTMipsCheriAbiFlags =
    BUILD_ENUM_STRINGS(ElfDynamicDTMipsCheriAbiFlagsDefs);

#undef LLVM_READOBJ_DT_FLAG_ENT

template <typename T, typename TFlag>
void printFlags(T Value, EnumStrings<TFlag> Flags, raw_ostream &OS) {
  SmallVector<const EnumString<TFlag>*, 10> SetFlags;
...
std::string ELFDumper<ELFT>::getDynamicEntry(uint64_t Type,
  ...
  case EM_MIPS:
    ...
    case DT_MIPS_CHERI_FLAGS: {
      ...
      if (Value) {
        Result += ' ';
        Result += FormatFlags(Value, ElfDynamicDTMipsCheriAbiFlags);
      }
      return Result;
    }
    default:
