...
TEST_F(IR2VecTestFixture, GetInstVecMap) {
  ...
  EXPECT_TRUE(InstMap.at(AddInst).approximatelyEquals(Embedding(2, 27.8)));
  EXPECT_TRUE(InstMap.at(RetInst).approximatelyEquals(Embedding(2, 16.9)));
}
...
TEST_F(IR2VecTestFixture, GetBBVecMap) {
  ...
  // {44.4, 44.4}
  EXPECT_TRUE(BBMap.at(BB).approximatelyEquals(Embedding(2, 44.7)));
}
...
TEST_F(IR2VecTestFixture, GetBBVector) {
  ...
  EXPECT_TRUE(BBVec.approximatelyEquals(Embedding(2, 44.7)));
}
...
TEST_F(IR2VecTestFixture, GetFunctionVector) {
  ...
  EXPECT_TRUE(FuncVec.approximatelyEquals(Embedding(2, 44.7)));
}

static constexpr unsigned MaxOpcodes = 67;
static constexpr unsigned MaxTypeIDs = 22;
