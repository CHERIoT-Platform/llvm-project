...
TEST(ComputeTargetABI, SelectsExpectedABI) {
  ...
  EXPECT_EQ(computeTargetABI("riscv32", "+xcheriot"), RISCVABI::ABI_CHERIOT_BAREMETAL);
}
...
TEST(ComputeTargetABI, ReportsInvalidExplicitABI) {
  ...
  EXPECT_EQ(computeTargetABIError("riscv32", "+e", "ilp32"),
            "only the ilp32e and il32pc64e ABIs are supported for RV32E");
  EXPECT_EQ(computeTargetABIError("riscv32", "+e,+xcheriot", "ilp32e"),
            "only the cheriot and cheriot-baremetal ABIs are supported for XCheriot");
  ...
  EXPECT_EQ(computeTargetABIError("riscv32", "", "il32pc64"),
            "pure-capability ABI can't be used for a target that doesn't support the XCheri instruction set extension");
