...
void IntrinsicEmitter::EmitIntrinsicToBuiltinMap(
  ...
  for (const CodeGenIntrinsic &Int : Ints) {
    ...
    *CommonPrefix = CommonPrefix->take_front(Mismatch - CommonPrefix->begin());
    ...
    if (!Int.GCCBuiltinAliasName.empty()) {
      StringRef GCCAliasName = Int.GCCBuiltinAliasName;
      if (!Map.insert({GCCAliasName, Int.EnumName}).second)
        PrintFatalError(Int.TheDef->getLoc(),
                        "Intrinsic '" + Int.TheDef->getName() +
                            "': duplicate GCC builtin name!");

      const char *Mismatch = mismatch(*CommonPrefix, GCCAliasName).first;
      *CommonPrefix = CommonPrefix->take_front(Mismatch - CommonPrefix->begin());
    }
  }
